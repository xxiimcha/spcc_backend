...
try {
  ...
  // ✅ Safer schedules query (no invalid WHERE)
  $hasSchedSy  = $colExists($conn, 'schedules', 'school_year');
  $hasSchedSem = $colExists($conn, 'schedules', 'semester');

  $schedWhereParts = [];
  if ($hasSchedSy)  $schedWhereParts[] = "s.school_year = '" . $esc($sy) . "'";
  if ($sem !== '' && $hasSchedSem) $schedWhereParts[] = "s.semester = '" . $esc($sem) . "'";
  $schedWhere = count($schedWhereParts) ? (' WHERE ' . implode(' AND ', $schedWhereParts)) : '';
  ...
  $schedulesSql = "
    SELECT
      ...
      s.time_end,
      s.room,
      " . ($hasSchedSem ? "s.semester" : "NULL AS semester") . ",
      " . ($hasSchedSy ? "s.school_year" : "NULL AS school_year") . "
    FROM schedules s
